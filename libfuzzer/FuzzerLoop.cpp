...
void Fuzzer::SetMaxInputLen(size_t MaxInputLen) {
  Printf("INFO: Fuzzer::SetMaxInputLen\n");
...
#define _HF_INPUT_FD 1000

int subprocess(char* shell_script){
  std::cout << __FUNCTION__ << std::endl;
  const char* content;
  std::ifstream file_stream(shell_script);
  ...
  ch = strtok((char*)content, " ");
  while (ch != NULL) {
    args[num++]=ch;
    //printf("%s\n", ch);
    ch = strtok(NULL, " ");
  }
  args[num++]="/dev/fd/1021";
  args[num]=nullptr;

  if(fork()==0){
    if(-1==execve(args[0],args,nullptr)){
